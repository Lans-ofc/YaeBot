...
let name = await conn.getName(who)
 ...
 const ini = await conn.sendContactArray(m.chat, [
    [`${nomorown3}`, `${await conn.getName(nomorown3+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `⁨al3271525@gmail.com`, `🇮🇩 Indonesia`, `🚀 https://lynk.id/na_offical`, `LansOfc`],
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't spam/call me 😢`, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://lynk.id/na_offical`, `🤖 Dont Forget To Donate!`]
  ], fkontak)
